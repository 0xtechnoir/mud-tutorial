...
export function createSystemCalls(
  ...
  const moveTo = async (inputX: number, inputY: number) => {
    ...
    try {
      ...
      // waits for txReduced$ stream to emit a transaction hash that matchs tx.hash
  ...
  const throwBall = async () => {
    ...
    console.log("Throw Ball called")
    const result = catchAttempt.result as MonsterCatchResult;
    console.log('result: ', MonsterCatchResult[result])
    return result
  };
